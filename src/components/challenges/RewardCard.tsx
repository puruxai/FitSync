// FitSync Component: Challenge Reward Card
// Displays challenge completion bonuses and XP point claims

import React from 'react';
import Card from '../ui/Card';
import Button from '../ui/Button';
import type { ChallengeReward } from '../../services/challengeReward';
  ...
  return (
    <Card variant="glass" className="p-4 flex items-center justify-between gap-4 text-left border border-slate-200/50 dark:border-slate-800/40 rounded-3xl">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${reward.is_claimed ? 'bg-slate-100 dark:bg-slate-850 text-slate-405' : 'bg-yellow-500/10 text-yellow-500'}`}>
          <span className="material-symbols-outlined text-[1.4em]">
            {reward.is_claimed ? 'check_circle' : 'military_tech'}
          </span>
        </div>
        <div>
          <h4 className="text-xs font-bold text-slate-850 dark:text-white leading-tight line-clamp-1">
            {challenge.title} Winner
          </h4>
          <p className="text-[10px] text-slate-400 font-semibold mt-1">
            Claim +{reward.reward_points} XP points
          </p>
        </div>
      </div>
